    ...
    queryFn: async (): Promise<ReactorData> => {
      ...
      if (snapshot) {
        if (snapshot.labour_pct > 32) {
          alerts.push({ id: "labour-high", level: "critical", title: "Labour above 32%", detail: `Currently ${snapshot.labour_pct.toFixed(1)}%`, module: "Labour" });
        } else if (snapshot.labour_pct > 28) {
          alerts.push({ id: "labour-warn", level: "warning", title: "Labour trending high", detail: `Currently ${snapshot.labour_pct.toFixed(1)}%`, module: "Labour" });
        }

        const foodPct = snapshot.revenue_total > 0 ? (snapshot.cogs_food / snapshot.revenue_total) * 100 : 0;
        if (foodPct > 35) {
          alerts.push({ id: "food-high", level: "critical", title: "Food cost above 35%", detail: `Currently ${foodPct.toFixed(1)}%`, module: "Food Cost" });
        }

        const suppliesPct = snapshot.ops_supplies_pct ?? 0;
        if (suppliesPct > 4) {
          alerts.push({ id: "supplies-high", level: "warning", title: "Supplies above 4%", detail: `Currently ${suppliesPct.toFixed(1)}%`, module: "Overheads" });
        }

        if (snapshot.net_profit_pct < 5) {
          alerts.push({ id: "net-low", level: "critical", title: "Net margin under 5%", detail: `Currently ${snapshot.net_profit_pct.toFixed(1)}%`, module: "P&L" });
        }
      }
