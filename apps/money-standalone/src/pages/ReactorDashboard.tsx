...
import {
  Atom, TrendingUp, AlertTriangle, CheckCircle2,
...
const MODULE_LABELS: Record<string, string> = {
  restos: "POS", reservationos: "POS", bevos: "Bev Cost", chefos: "Food Cost",
  labouros: "Labour", growthos: "Marketing", clockos: "Labour", overheados: "Overheads",
};
...
const ReactorDashboard = () => {
  ...
  const { orgId } = useAuth();
  const { data: reactor, isLoading } = useMoneyReactor(orgId ?? undefined);
  ...
  const d = {
    ...
    audit: { overall: reactor?.auditScore ?? 0 },
    alerts: (reactor?.alerts ?? []).map(a => ({ id: a.id, level: a.level, message: `${a.title} — ${a.detail}`, module: a.module })),
    ecosystem: (reactor?.ecosystem ?? []).map(e => ({
      module: MODULE_LABELS[e.module] || e.module,
  ...
  return (
    <div className="p-3 lg:p-5 space-y-4 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <Card>
          ...
          <CardContent className="px-4 pb-3 space-y-2">
            <div className="text-center">
              <p className="text-2xl font-bold font-mono text-foreground">${rev.toLocaleString()}</p>
        ...
        <Card>
          <CardHeader className="pb-1 pt-3 px-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" className="text-[10px] h-6 px-2" onClick={() => navigate("/settings")}>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          { label: "Full P&L", icon: BarChart3, href: "/pnl" },
          { label: "Alert Rules", icon: Bell, href: "/settings" },
          { label: "Trend Analysis", icon: TrendingUp, href: "/trends" },
        ].map((action) => {
