...
const STEPS = ["Setup", "Guests", "Voting", "Dashboard"];

export function SundayRoastFlow() {
  ...
  const stepIndex =
    ...
    currentView === "voting" ? 2 : 3;

  const handleBack = () => {
    lightTap();
    if (currentView === "invite") setView("setup");
    else if (currentView === "voting") setView("invite");
    else if (currentView === "dashboard") setView("voting");
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* Content */}
      {currentView === "setup" && <EventSetupForm eventType="sunday_roast" />}
      {currentView === "invite" && <GuestManager />}
      {currentView === "voting" && <VotingBoard />}
