      ...
      >
        ...
        <View style={{ flexDirection: "row", gap: 12, marginBottom: 16 }}>
          <View style={{
            ...
            backgroundColor: profile.current_streak >= 7 ? colors.warning + "20" : colors.surface,
            borderRadius: 12, padding: 12, borderWidth: 1, borderColor: colors.border,
          }}>
            <Flame size={20} color={profile.current_streak >= 7 ? colors.warning : colors.textMuted} strokeWidth={2} />
            <View>
          ...
          <View style={{
            flex: 1, flexDirection: "row", alignItems: "center", gap: 8,
            backgroundColor: profile.league === "pro" ? colors.accent + "20" : colors.surface,
            borderRadius: 12, padding: 12, borderWidth: 1, borderColor: colors.border,
          }}>
            <Trophy size={20} color={profile.league === "pro" ? colors.accent : colors.textMuted} strokeWidth={2} />
            <View>
        ...
        {/* Compliance Gate */}
        <View style={{
          backgroundColor: isUnlocked ? colors.success + "10" : colors.surface,
          borderRadius: 16, padding: 16, marginBottom: 20,
          borderWidth: 2, borderColor: isUnlocked ? colors.success : colors.border,
        }}>
          <Text style={{ fontSize: 13, fontWeight: "700", color: isUnlocked ? colors.success : colors.text, marginBottom: 12 }}>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            {gateItems.map((item) => (
              <View key={item.key} style={{
                ...
                backgroundColor: item.done ? colors.success + "15" : colors.background,
                borderWidth: 1, borderColor: item.done ? colors.success : colors.border,
              }}>
                {item.done ? (
                ...
                <Text style={{ fontSize: 9, fontWeight: "600", color: item.done ? colors.success : colors.textMuted, marginTop: 4, textAlign: "center" }}>
                  {item.label}
                </Text>
              </View>
            ))}
          ...
          {!isUnlocked && (
            <View style={{ height: 4, backgroundColor: colors.border, borderRadius: 2, marginTop: 12, overflow: "hidden" }}>
              <View style={{ height: 4, backgroundColor: colors.success, borderRadius: 2, width: `${Math.round((doneCount / 3) * 100)}%` as any }} />
            </View>
          )}
        </View>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 12, color: colors.textMuted }}>45s frenzy — slice flying onions mid-air</Text>
          </View>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 12, color: colors.textMuted }}>40s zen — calm a stray kitchen cat</Text>
          </View>
          <ChevronRight size={18} color={colors.textMuted} strokeWidth={2} />
        </Pressable>
