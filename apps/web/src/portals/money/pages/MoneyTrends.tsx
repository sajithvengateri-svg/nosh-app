...
import {
  ...
  Utensils, Megaphone, CalendarCheck, Activity, Loader2,
} from "lucide-react";
...
interface MetricConfig {
  ...
  prefix: string;
  source: string;
  extract: (s: PnLSnapshot) => number;
  higherIsBetter: boolean;
}

const METRIC_CONFIGS: MetricConfig[] = [
  { id: "revenue", label: "Revenue", icon: DollarSign, unit: "$", prefix: "$", source: "RestOS",
    extract: s => s.revenue_total, higherIsBetter: true },
  { id: "food_cost", label: "Food Cost %", icon: ChefHat, unit: "%", prefix: "", source: "ChefOS",
    extract: s => s.revenue_total > 0 ? (s.cogs_food / s.revenue_total) * 100 : 0, higherIsBetter: false },
  { id: "bev_cost", label: "Bev Cost %", icon: Wine, unit: "%", prefix: "", source: "BevOS",
    extract: s => s.revenue_total > 0 ? (s.cogs_bev / s.revenue_total) * 100 : 0, higherIsBetter: false },
  { id: "labour", label: "Labour %", icon: Users, unit: "%", prefix: "", source: "LabourOS",
    extract: s => s.labour_pct, higherIsBetter: false },
  { id: "prime_cost", label: "Prime Cost %", icon: BarChart3, unit: "%", prefix: "", source: "OverheadOS",
    extract: s => s.prime_cost_pct, higherIsBetter: false },
  { id: "net_profit", label: "Net Profit %", icon: Activity, unit: "%", prefix: "", source: "OverheadOS",
    extract: s => s.net_profit_pct, higherIsBetter: true },
  { id: "gross_margin", label: "Gross Margin %", icon: TrendingUp, unit: "%", prefix: "", source: "MoneyOS",
    extract: s => s.gross_margin_pct, higherIsBetter: true },
  { id: "overhead", label: "Overhead %", icon: BarChart3, unit: "%", prefix: "", source: "OverheadOS",
    extract: s => s.overhead_pct, higherIsBetter: false },
];

// ─── Demo fallback data ─────────────────────────────────
const generateDays = (base: number, variance: number, trend: number = 0) =>
...
// Weekly aggregation
const aggregateWeekly = (data: { day?: number; value: number }[]) => {
...
const CORRELATIONS = [
  ...
  { title: "No-Show ↔ Revenue Loss", desc: "Average no-show costs $96/seat. Deposit threshold recommendation: $25pp for 4+.", severity: "info" as const, metrics: ["No-Shows", "Revenue"] },
];
...
const MoneyTrends = () => {
  ...
  const { data: snapshots, isLoading } = useMoneyTrends(currentOrg?.id, Number(timeRange));

  // Build metric data from live snapshots or fallback to demo
    ...
    return METRIC_CONFIGS.map(cfg => {
      if (hasLive) {
        ...
        return {
          ...cfg,
          data,
          summary: { current: fmtVal, prior: fmtPrior, change: `${change >= 0 ? "+" : ""}${change.toFixed(1)}%`, positive },
          isLive: true,
        };
      }
  ...
  const chartConfig = {
    value: { label: metric.label, color: metric.summary.positive ? "hsl(145 60% 45%)" : "hsl(0 70% 55%)" },
  };
    ...
    <div className="p-3 lg:p-5 space-y-4 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{currentOrg?.name ?? "Your Venue"} · Deep metric analysis & cross-product correlations</p>
        </div>
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="trends" className="space-y-4 mt-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-2">
            {metricsData.map(m => {
              ...
              return (
                <Card key={m.id}
                  className={cn("cursor-pointer transition-all border",
                    selectedMetric === m.id ? "border-primary bg-primary/5" : "border-border/50 hover:border-primary/20"
                  )}
                ...
                >
                  <CardContent className="p-2.5 space-y-1">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className={cn("text-[10px] font-mono", m.summary.positive ? "text-emerald-500" : "text-rose-500")}>
                        {m.summary.change}
                      </span>
                    </div>
              ...
              <ChartContainer config={chartConfig} className="h-[280px] w-full">
                <AreaChart data={displayData}>
                  ...
                  <Area type="monotone" dataKey="value"
                    stroke={metric.summary.positive ? "hsl(145 60% 45%)" : "hsl(0 70% 55%)"}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              { label: `${timeRange}-Day Avg`, value: `${metric.prefix}${Math.round(metric.data.reduce((s, d) => s + d.value, 0) / metric.data.length)}${metric.unit === "%" ? "%" : metric.unit === "x" ? "x" : ""}` },
              ...
              <Card key={s.label}>
                <CardContent className="p-3 text-center">
                  ...
                  <p className={cn("text-lg font-bold font-mono",
                    s.positive !== undefined ? (s.positive ? "text-emerald-500" : "text-rose-500") : "text-foreground"
                  )}>{s.value}</p>
                </CardContent>
              </Card>
            ))}
        ...
        <TabsContent value="correlations" className="space-y-4 mt-4">
          <p className="text-xs text-muted-foreground">Cross-product correlations detected by hourly analysis of all ecosystem data.</p>
          <div className="space-y-3">
            {CORRELATIONS.map((c, i) => (
              <Card key={i} className={cn("border-l-4",
                c.severity === "warning" ? "border-l-amber-500" : c.severity === "positive" ? "border-l-emerald-500" : "border-l-blue-400"
              )}>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className={cn("text-[9px] h-4",
                          c.severity === "warning" ? "text-amber-500 border-amber-500/30" : c.severity === "positive" ? "text-emerald-500 border-emerald-500/30" : "text-blue-400 border-blue-400/30"
                        )}>
                      ...
                      <div className="flex gap-1.5 pt-1">
                        {c.metrics.map(m => (
                          <Badge key={m} variant="secondary" className="text-[9px] h-4 px-1.5">{m}</Badge>
                        ))}
                      </div>
                    </div>
