...
const AUTH_SOURCE = "gcc_food_safety";
...
const GCCFoodSafetyLanding = () => {
  useSEO(SEO["/food-safety-gcc"]);
  const navigate = useNavigate();
  const { sectionMap, isVisible, isLoading } = useGCCFoodSafetyLandingSections();
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm" : "bg-white/80 backdrop-blur-sm"}`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/food-safety-gcc")}>
            <Shield className="w-7 h-7 text-emerald-600" />
            <span className="text-xl font-bold text-gray-900">Food Safety <span className="text-emerald-600">OS</span></span>
          </div>
          <div className="hidden md:flex items-center gap-8">
            ...
            <Button size="sm" onClick={() => navigate(AUTH_SIGNUP)} className="bg-emerald-600 hover:bg-emerald-700 text-white">
      ...
      {isVisible("hero") && (
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
            ...
            <>
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-2">
                {images.map((_: any, i: number) => (
                  <button key={i} onClick={() => setSlide(i)} className={`w-2 h-2 rounded-full transition-all ${i === slide ? "w-6 bg-emerald-600" : "bg-gray-400/60"}`} />
                ))}
              </div>
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
            <FadeUp>
              {heroContent.badge && (
                <Badge className="mb-6 bg-emerald-100 text-emerald-700 border-emerald-200 px-4 py-1.5 text-sm">
                  {heroContent.badge}
                </Badge>
              )}
            </FadeUp>
            <FadeUp delay={0.1}>
              <h1 className="text-5xl md:text-7xl font-bold leading-tight mb-6 text-gray-900">
                {sectionMap.hero?.title || "Food Safety Compliance for the Gulf"}
              </h1>
            </FadeUp>
            <FadeUp delay={0.2}>
            ...
            <FadeUp delay={0.3}>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button size="lg" onClick={() => navigate(AUTH_SIGNUP)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-6 text-lg shadow-lg shadow-emerald-600/20">
              ...
              {arr("compliance_features").map((feat: any, i: number) => {
                ...
                return (
                  <FadeUp key={i} delay={i * 0.1}>
                    <div className="bg-white rounded-xl p-6 border border-gray-200 hover:border-emerald-300 hover:shadow-lg transition-all group">
      ...
      {/* Dubai Municipality / SFDA Section */}
      {isVisible("dubai_municipality") && (
        <section id="compliance" className="py-24 px-6">
          <div className="max-w-4xl mx-auto text-center">
            <FadeUp>
              <h2 className="text-4xl font-bold mb-4 text-gray-900">{sectionMap.dubai_municipality?.title}</h2>
              <p className="text-lg text-gray-600 mb-12">{sectionMap.dubai_municipality?.subtitle}</p>
            </FadeUp>
            <FadeUp delay={0.2}>
              <div className="flex flex-wrap justify-center gap-4 mb-8">
                {(obj("dubai_municipality").compliance_levels || []).map((level: string, i: number) => (
                  <div key={i} className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-full px-5 py-3">
            ...
            <FadeUp delay={0.3}>
              <p className="text-gray-600 max-w-2xl mx-auto">{obj("dubai_municipality").description}</p>
            </FadeUp>
          </div>
        </section>
      )}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {arr("pricing").map((tier: any, i: number) => (
                <FadeUp key={i} delay={i * 0.1}>
                  <div className={`relative bg-white rounded-xl p-8 border ${tier.popular ? "border-emerald-500 shadow-xl shadow-emerald-500/10 ring-1 ring-emerald-500" : "border-gray-200"}`}>
                    {tier.popular && (
                    ...
                    <Button
                      className={`w-full ${tier.popular ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-900"}`}
      ...
      {isVisible("final_cta") && (
        <section className="py-24 px-6 bg-gray-50">
          <FadeUp>
            <div className="max-w-3xl mx-auto text-center bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-2xl p-12 shadow-xl">
              ...
              <Button
                ...
                className="bg-white text-emerald-700 hover:bg-emerald-50 px-8 py-6 text-lg font-semibold shadow-lg"
              >
      ...
      <CrossMarketFooter region="gcc" current="foodsafety" />
      ...
      <footer className="border-t border-gray-200 py-8 px-6 bg-white">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-500">Food Safety OS — GCC</span>
          </div>
          <p className="text-xs text-gray-400">&copy; {new Date().getFullYear()} ChefOS. All rights reserved.</p>
          {isAdmin && (
            <button onClick={() => navigate("/admin/gcc-food-safety-landing")} className="opacity-20 hover:opacity-60 transition-opacity">
...
export default GCCFoodSafetyLanding;
