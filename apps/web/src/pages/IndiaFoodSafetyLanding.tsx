...
const AUTH_SOURCE = "india_food_safety";
const AUTH_SIGNUP = `/auth?tab=signup&source=${AUTH_SOURCE}`;
...
const IndiaFoodSafetyLanding = () => {
  useSEO(SEO["/food-safety-india"]);
  const navigate = useNavigate();
  const { sectionMap, isVisible, isLoading } = useIndiaFoodSafetyLandingSections();
    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm" : "bg-white/80 backdrop-blur-sm"}`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/food-safety-india")}>
            <Shield className="w-7 h-7 text-orange-600" />
            <span className="text-xl font-bold text-gray-900">Food Safety <span className="text-orange-600">OS</span></span>
          </div>
          <div className="hidden md:flex items-center gap-8">
            <a href="#features" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">Features</a>
            <a href="#compliance" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">FSSAI</a>
            <a href="#pricing" className="text-sm text-gray-500 hover:text-gray-900 transition-colors">Pricing</a>
            <Button size="sm" onClick={() => navigate(AUTH_SIGNUP)} className="bg-orange-600 hover:bg-orange-700 text-white">
      ...
      {isVisible("hero") && (
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
            ...
            <>
              ...
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-2">
                {images.map((_: any, i: number) => (
                  <button key={i} onClick={() => setSlide(i)} className={`w-2 h-2 rounded-full transition-all ${i === slide ? "w-6 bg-orange-600" : "bg-gray-400/60"}`} />
                ))}
              </div>
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
            <FadeUp>
              {heroContent.badge && (
                <Badge className="mb-6 bg-orange-100 text-orange-700 border-orange-200 px-4 py-1.5 text-sm">
                  {heroContent.badge}
                </Badge>
              )}
            </FadeUp>
            <FadeUp delay={0.1}>
              <h1 className="text-5xl md:text-7xl font-bold leading-tight mb-6 text-gray-900">
                {sectionMap.hero?.title || "FSSAI Compliance, Simplified"}
              </h1>
            </FadeUp>
            <FadeUp delay={0.2}>
            ...
            <FadeUp delay={0.3}>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button size="lg" onClick={() => navigate(AUTH_SIGNUP)} className="bg-orange-600 hover:bg-orange-700 text-white px-8 py-6 text-lg shadow-lg shadow-orange-600/20">
              ...
              {arr("compliance_features").map((feat: any, i: number) => {
                ...
                return (
                  <FadeUp key={i} delay={i * 0.1}>
                    <div className="bg-white rounded-xl p-6 border border-gray-200 hover:border-orange-300 hover:shadow-lg transition-all group">
      ...
      {isVisible("fssai") && (
        <section id="compliance" className="py-24 px-6">
          <div className="max-w-4xl mx-auto text-center">
            <FadeUp>
              <h2 className="text-4xl font-bold mb-4 text-gray-900">{sectionMap.fssai?.title}</h2>
              <p className="text-lg text-gray-600 mb-12">{sectionMap.fssai?.subtitle}</p>
            </FadeUp>
            <FadeUp delay={0.2}>
              <div className="flex flex-wrap justify-center gap-4 mb-8">
                {(obj("fssai").compliance_levels || []).map((level: string, i: number) => (
                  <div key={i} className="flex items-center gap-2 bg-orange-50 border border-orange-200 rounded-full px-5 py-3">
            ...
            <FadeUp delay={0.3}>
              <p className="text-gray-600 max-w-2xl mx-auto">{obj("fssai").description}</p>
            </FadeUp>
          </div>
        </section>
      )}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {arr("pricing").map((tier: any, i: number) => (
                <FadeUp key={i} delay={i * 0.1}>
                  <div className={`relative bg-white rounded-xl p-8 border ${tier.popular ? "border-orange-500 shadow-xl shadow-orange-500/10 ring-1 ring-orange-500" : "border-gray-200"}`}>
                    {tier.popular && (
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                        <Badge className="bg-orange-600 text-white px-3">Most Popular</Badge>
                      </div>
                    )}
                    ...
                    <Button
                      className={`w-full ${tier.popular ? "bg-orange-600 hover:bg-orange-700 text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-900"}`}
                      onClick={() => navigate(AUTH_SIGNUP)}
                    >
      ...
      {isVisible("final_cta") && (
        <section className="py-24 px-6 bg-gray-50">
          <FadeUp>
            <div className="max-w-3xl mx-auto text-center bg-gradient-to-br from-orange-600 to-orange-800 rounded-2xl p-12 shadow-xl">
              <h2 className="text-3xl font-bold mb-4 text-white">{sectionMap.final_cta?.title}</h2>
              <p className="text-orange-100 mb-8">{sectionMap.final_cta?.subtitle}</p>
              <Button
                size="lg"
                onClick={() => navigate(obj("final_cta").button_link || AUTH_SIGNUP)}
                className="bg-white text-orange-700 hover:bg-orange-50 px-8 py-6 text-lg font-semibold shadow-lg"
              >
      ...
      <CrossMarketFooter region="india" current="foodsafety" />
      ...
      <footer className="border-t border-gray-200 py-8 px-6 bg-white">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-500">Food Safety OS — India</span>
          </div>
          <p className="text-xs text-gray-400">&copy; {new Date().getFullYear()} ChefOS. All rights reserved.</p>
          {isAdmin && (
            <button onClick={() => navigate("/admin/india-food-safety-landing")} className="opacity-20 hover:opacity-60 transition-opacity">
...
export default IndiaFoodSafetyLanding;
