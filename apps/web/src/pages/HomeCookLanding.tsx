...
const lifestyleImages = [
  { src: cupcakesImg, alt: "Pink cupcakes with blue sprinkles", accent: "pink" },
  { src: pantryImg, alt: "Organised home pantry", accent: "sky" },
  { src: bakesImg, alt: "Fresh-baked croissants", accent: "pink" },
  { src: macaronsImg, alt: "Colourful French macarons", accent: "sky" },
];

const HomeCookLanding = () => {
  useSEO(SEO["/home-cook"]);
  const { sectionMap, isVisible, isLoading } = useHomeCookLandingSections();

  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-pink-50 to-white dark:from-neutral-950 dark:to-neutral-950">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-50 via-sky-50/30 to-white dark:from-neutral-950 dark:via-neutral-900 dark:to-neutral-950">
      {/* Nav */}
      <header className="sticky top-0 z-50 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-lg border-b border-pink-100 dark:border-neutral-800">
        <div className="max-w-5xl mx-auto flex items-center justify-between px-6 py-4">
          <Link to="/home-cook" className="flex items-center gap-2 cursor-pointer">
            <Heart className="w-6 h-6 text-pink-500" />
            <span className="font-bold text-lg text-foreground">ChefOS <span className="text-pink-500">Home</span></span>
          </Link>
          <div className="flex items-center gap-3">
            <Link to="/auth?mode=home_cook&tab=login&source=home_cook">
              <Button variant="ghost" size="sm">Log In</Button>
            </Link>
            <Link to="/auth?mode=home_cook&tab=signup&source=home_cook">
              <Button size="sm" className="bg-pink-500 hover:bg-pink-600 text-white">Get Started</Button>
            </Link>
          </div>
        </div>
      </header>
      ...
      {isVisible("hero") && hero && (
        <section className="relative overflow-hidden min-h-[70vh] flex items-center justify-center">
          ...
          {/* Gradient overlay */}
          <div className="absolute inset-0 bg-gradient-to-b from-pink-500/10 via-white/70 to-white dark:from-pink-950/30 dark:via-neutral-950/80 dark:to-neutral-950" />
          ...
          >
            <div className="bg-white/70 dark:bg-neutral-900/70 backdrop-blur-xl rounded-3xl p-8 sm:p-12 shadow-xl border border-pink-100/50 dark:border-neutral-700/50">
              {heroContent.badge && (
                <motion.div
                ...
                >
                  <Badge className="mb-6 bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400 border-pink-200 dark:border-pink-800">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground leading-tight">
                {hero.title.includes(",") ? (
                  <>
                    ...
                    <span className="text-pink-500">{hero.title.split(",").slice(1).join(",").trim()}</span>
                  </>
                ) : (
                  hero.title
                )}
              ...
              <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
                <Link to="/auth?mode=home_cook&tab=signup&source=home_cook">
                  ...
                  >
                    <Button size="lg" className="bg-pink-500 hover:bg-pink-600 text-white px-8 text-lg shadow-lg shadow-pink-500/20">
      ...
      <section className="max-w-5xl mx-auto px-6 py-16">
        <motion.div {...fadeUp} className="text-center mb-10">
          ...
          <p className="mt-2 text-muted-foreground">The tech the pros use — made easy for your kitchen.</p>
        </motion.div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuresContent.map((f: any, i: number) => {
              ...
              return (
                ...
                >
                  <Card className={`h-full rounded-2xl border-${isPink ? "pink" : "sky"}-100 dark:border-neutral-800 hover:shadow-lg transition-shadow`}>
            ...
            {comingSoonContent.map((item: any, i: number) => {
              ...
              return (
                <motion.div key={i} {...fadeUp} transition={{ delay: i * 0.1 }}>
                  <Card className="h-full border-dashed border-pink-200 dark:border-neutral-700 opacity-80 rounded-2xl">
                    <CardContent className="p-6 text-center">
                      <Icon className="w-8 h-8 text-sky-400 mx-auto mb-3" />
                      <h3 className="font-semibold text-foreground">{item.title}</h3>
                      <p className="mt-1 text-xs text-muted-foreground">{item.desc}</p>
                      <Badge variant="secondary" className="mt-3 text-[10px] bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400">Coming Soon</Badge>
                    </CardContent>
                  </Card>
                </motion.div>
              );
      ...
      {isVisible("money_tracker") && moneyTracker && (
        <section className="max-w-5xl mx-auto px-6 py-16">
          <motion.div {...fadeUp} className="bg-gradient-to-br from-pink-50 to-sky-50 dark:from-neutral-900 dark:to-neutral-800 rounded-3xl p-8 sm:p-12 text-center">
            ...
            {Array.isArray(moneyContent.metrics) && moneyContent.metrics.length > 0 && (
              <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-md mx-auto">
                {moneyContent.metrics.map((label: string) => (
                  <div key={label} className="bg-white dark:bg-neutral-900 rounded-xl p-4 shadow-sm border border-pink-100/50 dark:border-neutral-700">
      ...
      {isVisible("solo_team") && soloTeam && (
        <section className="max-w-5xl mx-auto px-6 py-16 text-center">
          <motion.div {...fadeUp}>
            <Users2 className="w-10 h-10 text-sky-500 mx-auto mb-4" />
      ...
      {isVisible("themes") && themes && themesContent.length > 0 && (
        <section className="max-w-5xl mx-auto px-6 py-16 text-center">
          <motion.div {...fadeUp}>
            <Palette className="w-10 h-10 text-pink-500 mx-auto mb-4" />
      ...
      {isVisible("final_cta") && finalCta && (
        <section className="max-w-5xl mx-auto px-6 py-20 text-center">
          <motion.div {...fadeUp}>
            ...
            <Link to="/auth?mode=home_cook&tab=signup&source=home_cook">
              <Button size="lg" className="mt-8 bg-pink-500 hover:bg-pink-600 text-white px-8 text-lg shadow-lg shadow-pink-500/20">
      ...
      <CrossMarketFooter region="au" current="homecook" />

      {/* Footer */}
      <footer className="border-t border-pink-100 dark:border-neutral-800 py-8">
        <div className="max-w-5xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <Heart className="w-4 h-4 text-pink-500" />
            <span className="text-sm text-muted-foreground">ChefOS Home</span>
          </div>
          <div className="flex items-center gap-3">
...
export default HomeCookLanding;
